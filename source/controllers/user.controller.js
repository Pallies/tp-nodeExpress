...
export const updateUser = async (req, res) => {
  ...
  Models.UserModel.findOneAndUpdate({_id:userModel._id}, req.body)
    .then((resp) => res.json(resp))
    .catch((err) => res.status(400).send(err));
...
export const updateUserById = async (req, res) => {
  ...
  const responseObjectById = function (){
     Models.UserModel.findById(option).then(result=>res.json(result))
  }
  await Models.UserModel.findOneAndUpdate(option, req.body)
  .then(responseObjectById)
  .catch((err) => res.status(400).send(err));
  // retourne l'objet modifié
};
export const deleteUser = async (req, res) => {
  const response=function(){res.json({message: `suppression réussi pour l'objet ${req.body._id||req.body.name}`})}
  const responseError=function(){res.send({message: `problème de suppression de l'objet ${req.body._id||req.body.name}`})}
  delete req.body._id // suppression de l'id
  const deleteUserById = function(){
    Models.UserModel.findOneAndDelete(userModel._id, req.body)
    .then(response)
    .catch(responseError)
  }
  const userModel = await Models.UserModel.findOne(req.body) //recherche élément pour l'id
